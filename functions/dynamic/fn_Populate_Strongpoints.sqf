//
//  [_Trigger,_Side,_InfantryNumber,_CreateObjective,_CreateLocalPatrols] spawn OKS_fnc_Populate_Strongpoints;
//
...
if(_CountStrongpoints > 0) then {
	...
	if(_Compounds isNotEqualTo []) then {
		{
			...
			if(_EnableEnemyMarkers && _Side != civilian) then {
				[_Compound,_Side,"infantry",_GarrisonNumber,0.8] spawn OKS_fnc_CreateUnitMarker;
			};
			[_GarrisonNumber,getPos _X,_Side,_UnitArray,_CompoundSize] spawn OKS_fnc_Garrison_Compound;
			if(_CreateLocalPatrols) then {
				if(_Debug_Variable) then {
			...
			if(_CreateObjective && {_X getVariable ["isSectorTrigger",false]} count (_X nearObjects ["EmptyDetector",(_CompoundSize * 3)]) < 1) then {
				[getPos _X,"sector",(_CompoundSize * 2),_Side,_CreateLocalPatrols] spawn OKS_fnc_CreateObjectives;
			};
			sleep 3;
		} foreach _Compounds;
	};
	...
	if(_Strongholds isNotEqualTo []) then {
		{
			...
			if(_CreateObjective) then {
				[_X,"neutralize",100,_Side,_CreateLocalPatrols] spawn OKS_fnc_CreateObjectives;
			};
			sleep 3;
...

	{
		...
		if(_CreateObjective && {_X getVariable ["isSectorTrigger",false]} count (_X nearObjects ["EmptyDetector", 250]) < 1) then {
			[_MarkerPos,"sector",125,_Side,_CreateLocalPatrols] spawn OKS_fnc_CreateObjectives;
		};
		...
		if(_CreateLocalPatrols) then {
			...
			if(count _SortedBuildings > 0) then {
				...
				{
					[4,_X,_Side,_UnitArray] spawn OKS_fnc_Garrison;
					if(_EnableEnemyMarkers && _Side != civilian) then {
						_Marker = createMarker [format ["oks_SP_Marker_%1",str round(random 90000)],getPos _X];
						[_marker,_Side,"infantry",4,0.8] spawn OKS_fnc_CreateUnitMarker;
					};
