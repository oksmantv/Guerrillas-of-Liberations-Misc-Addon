/*
	...
	[Position/Object,TypeOfObjective,Range,EAST,Patrols?,PlaceObjects?,OverrideTaskNotification?] spawn OKS_fnc_CreateObjectives;
...
switch (_TypeOfObjective) do {
		...
		switch (typename _Object) do {
			case "OBJECT": {
				...
				if(isNil "_TargetGroup") then {
					...
					_Group = [_GarrisonMaxSize,_House,_Side,_UnitArray] call OKS_fnc_Garrison;
					_TargetGroup = _Group;
				};
			};
			case "GROUP": {
	...
	case "cache": {
		...
		if(_ObjectivePatrols) then {
			[_Crate getPos [25,(random 360)],5,150,_Side] spawn OKS_fnc_Patrol_Spawn;
		};
	};
	...
	case "motorpool": {
		...
		if(_ObjectivePatrols) then {
			[_Crate getPos [25,(random 360)],4,150,_Side] spawn OKS_fnc_Patrol_Spawn;
			[_Crate getPos [25,(random 360)],4,150,_Side] spawn OKS_fnc_Patrol_Spawn;
		};
	};
	...
	case "radiotower": {
		...
		if(_ObjectivePatrols) then {
			[_SpawnPos getPos [25,(random 360)],3,150,_Side] spawn OKS_fnc_Patrol_Spawn;
			[_SpawnPos getPos [25,(random 360)],3,150,_Side] spawn OKS_fnc_Patrol_Spawn;
		};
		if(_EnableObjectiveTasks) then {
	...
	case "artillery": {
		...
		if(_EnableObjectiveTasks) then {
			...
			_Task = [true,format["OKS_Artillery_Objective_%1",(round(random 9000))], ["The Enemy have set up artillery positions that shell nearby friendly positions. Locate the artillery and spike it! Destroy to complete.", "Destroy Artillery", "Destroy Artillery"], getPos _Arty,"AUTOASSIGNED",-1,false] call BIS_fnc_taskCreate;
				...
				if(_ObjectivePatrols) then {
					[_Arty getPos [25,(random 360)],5,150,_Side] spawn OKS_fncPatrol_Spawn;
				};
		...
		if(!isNil "OKS_ArtyFire") then {
			...
			[_Side,_Arty,_Target,8,300,30] remoteExec ["OKS_fnc_ArtyFire",0];
		} else {
	...
	case "hostage": {
		...
		if(_ObjectivePatrols) then {
			[_House getPos [35,(random 360)],3,120,_Side] spawn OKS_fnc_Patrol_Spawn;
			[_House getPos [35,(random 360)],3,120,_Side] spawn OKS_fnc_Patrol_Spawn;
		};	
