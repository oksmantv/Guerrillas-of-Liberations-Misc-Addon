/*
	[Trigger,SizeNumber,_Side] spawn OKS_fnc_Find_Mortars;
	[Trigger_1,35,_Side] spawn OKS_fnc_Find_Mortars;
*/
...
_Debug_Variable = false;
	...
	While {true} do {
		...
		if(_Debug_Variable) then {
			systemChat format ["Find Mortar: %1 %2 %3 %4 %5",
				!(_SelectedPos isEqualTo [0,0,0]),
				{_SelectedPos Distance _X < 1000} count OKS_Mortar_Positions < 1,
				_SelectedPos inArea _MainTrigger,
				{_SelectedPos distance _X < 200} count OKS_Objective_Positions < 1,
				{_SelectedPos distance _X < 200} count OKS_RoadBlock_Positions < 1];
		};
		if(
			!(_SelectedPos isFlatEmpty  [-1, -1, 0.05, 45, 0] isEqualTo []) &&
			!(_SelectedPos isEqualTo [0,0,0]) &&
			{_SelectedPos Distance _X < 1000} count OKS_Mortar_Positions < 1 &&
			_SelectedPos inArea _MainTrigger &&
			{_SelectedPos distance _X < 200} count OKS_Objective_Positions < 1 &&
	...
	if(_MortarPatrol) then {
		[_SelectedPos,5,50,_Side] spawn OKS_fnc_Patrol_Spawn;
	};

	[_SelectedPos,_Side,(random 4),15] spawn OKS_fnc_Populate_Sandbag;
	...
	{_X setPos [getPos _X select 0,getpos _X select 1,0]} foreach _Objects;
		...
		[_SelectedPos,_Side,3,15] spawn OKS_fnc_Populate_Sandbag;
