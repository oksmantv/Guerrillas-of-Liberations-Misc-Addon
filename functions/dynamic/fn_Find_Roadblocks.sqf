/*
	[Trigger_1,15,EAST,true,true,1] spawn OKS_fnc_Find_Roadblocks;
*/
...
For "_i" from 1 to _RoadblockCount do {
	...
	if(count _LocationsInArea > 0) then {
		...
		while {_Condition} do {
			...
			if(_OnlyTarmac) then {
				if(!((getRoadInfo _Road select 0) isEqualTo "TRAIL") && !(getPos _Road isFlatEmpty  [-1, -1, 0.15, 25, 0] isEqualTo []) && {_Road Distance _X < 300} count OKS_RoadBlock_Positions < 1 && (getPos _Road) inArea _MainTrigger &&
				[_Road] call OKS_fnc_CheckRoadConnections && [_Road] call OKS_fnc_CheckIfTarmac) exitWith { if(_Debug_Variable) then {
					systemChat format ["Road Found: %1",_Road]}; _Condition = false};
			} else {
				if(!((getRoadInfo _Road select 0) isEqualTo "TRAIL") &&
					!(getPos _Road isFlatEmpty  [-1, -1, 0.05, 15, 0] isEqualTo []) &&
					{_Road Distance _X < 300} count OKS_RoadBlock_Positions < 1 &&
					(getPos _Road) inArea _MainTrigger &&
					[_Road] call OKS_fnc_CheckRoadConnections) exitWith {
						if(_Debug_Variable) then {
							systemChat format ["Non Tarmac Road Found: %1",_Road]
						};
					_Condition = false
				};
			};
	...
	if(!isNil "_Road") then {
		...
		*/
		...
		if(_RoadblockPatrols) then {
			[_RoadPos,5,150,_Side] spawn OKS_fnc_Patrol_Spawn;
		};
	...
	if(_Debug_Variable) then {
		...
		waitUntil{sleep 10; !isNil "OKS_fnc_Populate_Sandbag"};
		[getMarkerPos _marker,_Side,3 + (random 2),35] spawn OKS_fnc_Populate_Sandbag;
