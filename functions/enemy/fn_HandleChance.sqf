...
if (_dice < _adjustedChance) then {
    if(_surrenderDebug) then {
        format ["[SURRENDER] Surrendered: Dice rolled: %1 << %2", round(_dice * 100), round(_adjustedChance * 100)] spawn OKS_fnc_LogDebug;
    };   
    ...
    if(_surrenderDebug) then { 
        format ["[SURRENDER] No Surrender: Dice rolled: %1 << %2", round(_dice * 100), round(_adjustedChance * 100)] spawn OKS_fnc_LogDebug;
    };
};