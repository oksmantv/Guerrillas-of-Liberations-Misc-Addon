/*
    Code to deploy Recon Drone.
*/

private _args = _this;
if (_args isEqualType objNull) then {
    _args = [_args];
};

if (_args isEqualType []) then {
    if ((count _args) == 1 && {(_args select 0) isEqualType []}) then {
        _args = _args select 0;
    };

    if ((count _args) >= 1 && {(_args select 0) isEqualType []}) then {
        private _first = _args select 0;
        if ((count _first) >= 1 && {(_first select 0) isEqualType objNull}) then {
            _args = [(_first select 0)] + (_args select [1]);
        };
    };
};

_args params [["_player", objNull, [objNull]]];

[
    {
        _player = _this select 0;
        private _item = "GOL_Packed_Drone_Recon";
        private _sideKey = switch (side group _player) do {
            case WEST: {"BLUFOR"};
            case EAST: {"OPFOR"};
            case independent: {"INDEPENDENT"};
            default {"BLUFOR"};
        };
        private _type = missionNamespace getVariable [format ["GOL_DroneReconClass_%1", _sideKey], "B_UAV_01_F"];
        if (primaryWeapon _player != "") then {
            _player playMoveNow "AmovPknlMstpSlowWrflDnon";
        };

        private _actionName = "Deploying Recon Drone...";
        [
            _actionName,
            1.5,
            {true},
            {
                (_this select 0) params ["_player","_item","_type"];
                private _dir = (getDir _player);
                private _pos = (_player getRelPos [2, 0]);
                _pos set [2, ((getPosATL _player) select 2)];	
                _HasItem = [_player, _item, true] call BIS_fnc_hasItem;

                if (_HasItem && !(_type isEqualTo "")) then {
                    private _object = createVehicle [_type, [0,0,0], [], 0, "NONE"];
                    _object disableTIEquipment true;
                    _object setVariable ["A3TI_Disable", true,true];
                    _object setVariable ["GOL_ItemPacked",_item];
                    _player removeItem _item;
                    _DroneCrew = createVehicleCrew _object;
                    _player connectTerminalToUAV _object;					
                    _object setDir _dir;
                    _object setPosATL _pos;
                    _object spawn { 
                        sleep 5;
                        params ["_object"];
                        _object disableTIEquipment true;
                        _object setVariable ["A3TI_Disable", true,true];
                    };                   
                    ["ace_interact_menu_newControllableObject", [typeOf _object]] call CBA_fnc_globalEvent;
                }
            },
            {
                systemChat "Cancelled deploying Recon Drone.."
            },
            [_player,_item,_type]
        ] call CBA_fnc_progressBar;	
    },
    [_player]
] call CBA_fnc_execNextFrame;
