...
if (_Spinner) then
{
	...
	if (IsNil "NEKY_Hints") then
	{
		...
		if (((Player distance _Veh) <= 15) or (_Driver IsEqualTo Player)) then {[_Msg, 10] Spawn OKS_fnc_Hints};
		if !(_Driver isEqualTo ObjNull) then { [_Msg] RemoteExec ["OKS_fnc_Hints",_Veh] };
	};
};