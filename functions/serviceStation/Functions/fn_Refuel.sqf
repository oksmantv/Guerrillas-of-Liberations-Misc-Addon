//	[Vehicle, ServiceStation, FullService] spawn NEKY_ServiceStation_Refuel;
//	
//	This handles refueling a vehicle
//	
//	Made by NeKo-ArroW

Private ["_Veh","_SS","_FullService","_Refueling","_Msg","_FuelLimit"];
#include "..\fn_Service_Settings.sqf"

Params ["_Veh","_SS","_FullService"];

if !(_FullService) then {NEKY_ServiceStationActive PushBack _SS; PublicVariable "NEKY_ServiceStationActive"};
ScopeName "Main";
_FuelLimit = 0.75;
if(_Veh isKindOf "LandVehicle") then {
	_FuelLimit = 0.4;
};

if (fuel _Veh < _FuelLimit) then
{
	_MsgIndex = 0;
	while {fuel _veh < _FuelLimit} do
	{
		["Refueling", _Veh, true, _MsgIndex] spawn OKS_fnc_Hints;
		if (!(_Veh in NEKY_ServiceStationArray)) Then {BreakTo "Main"};
		_veh setfuel (fuel _veh + _Refueling);
		if (_MsgIndex isEqualTo 2) then {_MsgIndex = 0} else {_MsgIndex = _MsgIndex +1};
		sleep 1;
	};
	["Vehicle has been refueled.", _Veh] spawn OKS_fnc_Hints;
} else { ["Refueling not needed", _Veh] spawn OKS_fnc_Hints };
sleep 2;

if (!(_Veh in NEKY_ServiceStationArray)) exitWith {[_SS,true] call OKS_Fnc_Available; ["You have left the service station, service ending",_Veh] call OKS_Fnc_Hints};
if !(_FullService) then {[_SS,false] call OKS_Fnc_Available} else {[_Veh,_SS,true] spawn OKS_Fnc_Rearm};