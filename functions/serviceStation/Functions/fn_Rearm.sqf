...
if !(((Count _Turrets) isEqualTo 0) && ((Count _Pylons) isEqualTo 0)) then
{
	...
	{
		_Full = [];
		["Rearming", _Veh, true, _MsgIndex] spawn OKS_Fnc_Hints;
	...
	if (_Veh in NEKY_ServiceStationArray) then {[[_Veh],{(_This select 0) setVehicleAmmo 1}] remoteExec ["BIS_FNC_SPAWN",0]; ["Rearming Complete!", _Veh] spawn OKS_fnc_Hints};
} else {
	["Default Rearming not needed", _Veh] spawn OKS_Fnc_Hints;
	Sleep 2;
};
...
/// Drongos APS
if(!isNil "DAPS_fnc_RearmAPS") then {
...
if (
	...
	["Primary Ammunition Added", _Veh] spawn OKS_Fnc_Hints;
	sleep 2;
};
...
if(
	...
	_Veh AddMagazineCargo [_ammoClass,_rearm];
	["Secondary Ammunition Added", _Veh] spawn OKS_Fnc_Hints;
	sleep 2;
};
...
if (_FullService) then {["Service complete!", _Veh] spawn OKS_Fnc_Hints};
