...
ScopeName "Main";
_Veh engineOn False;
["Inspecting Vehicle.", _Veh] spawn OKS_Fnc_Hints;
...
if !((Count _DamagePoints) isEqualTo 0) then
{
	...
	for "_i" from 0 to ((count _DamagePoints) -1) do
	{
		...
		if (_PartHP >= 0.01) then
		{
			...
			{
				["Repairing", _Veh, true, _MsgIndex] spawn OKS_Fnc_Hints;
				if (!(_Veh in NEKY_ServiceStationArray)) then {BreakTo "Main"};
			...
			[(format ["%1 has been repaired.",_PartName]), _Veh] spawn OKS_Fnc_Hints;
			sleep 2;
		};
	};
	...
	if (!(_Veh in NEKY_ServiceStationArray)) Then {BreakTo "Main"};
	["Applying finishing Touches", _Veh] spawn OKS_Fnc_Hints;
	sleep 5;

	_Veh setdamage 0;
	["Repairs Complete.", _Veh] spawn OKS_Fnc_Hints;
	sleep 2;
} else {
	["Vehicle is not in need of repairs. Failsafe running.", _Veh] spawn OKS_Fnc_Hints;
	_Veh setDamage 0;
	Sleep 2;
};
if (!(_Veh in NEKY_ServiceStationArray)) exitWith {[_SS,true] call OKS_Fnc_Available; ["You have left the service station, service ending",_Veh] call OKS_Fnc_Hints};
if (((driver _Veh) in _Veh) && (!_FullService)) then {_Veh engineOn True};
if !(_FullService) then {[_SS,false] call OKS_Fnc_Available} else {[_Veh,_SS,True] call OKS_Fnc_Refuel};