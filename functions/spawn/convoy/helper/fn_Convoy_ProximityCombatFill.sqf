/*
    ...
    Vehicles are marked with setVariable ["CombatActivated", true, true] to avoid repeats.
...
if (!(_detector getVariable ["CombatActivated", false])) then {
    _toCheck pushBack _detector;
};

while {count _toCheck > 0} do {
    private _current = _toCheck deleteAt 0;
    if (!(_current getVariable ["CombatActivated", false])) then {
        _current setBehaviour "COMBAT";
        _current setVariable ["CombatActivated", true, true];
        _combatSet pushBack _current;
        // Find all unmarked vehicles within range
        private _nearby = _convoyArray select {
            _x != _current && { !(_x getVariable ["CombatActivated", false]) } && { _x distance _current <= _range }
        };
