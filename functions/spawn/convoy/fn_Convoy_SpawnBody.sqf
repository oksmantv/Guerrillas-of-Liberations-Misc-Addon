/*
	...
	12 - Task Array (Optional. If defined, spawns a moving convoy task via OKS_fnc_Convoy_TaskTracker.)
		Structure:
	...
	Task ID retrieval (param 12 must be defined):
		The task ID is stored asynchronously on the Final Waypoint object (param 3)
		under the variable "OKS_Convoy_TaskId" once the tracker initialises (~2s after spawn).

		Example:
		[convoy_1,convoy_2,convoy_3,east,[4,["rhs_btr60_msv"],6,25],[true,6],[],false,false,["rush"],"alternate",["",true,true,"destroy"]] spawn OKS_fnc_Convoy_Spawn;
		waitUntil { sleep 0.5; (convoy_3 getVariable ["OKS_Convoy_TaskId", ""]) != "" };
...
if (!isNil "_TaskArray" && {_TaskArray isEqualType []} && {count _TaskArray > 0}) then {
	[_VehicleArray, _TaskArray, _End, _ConvoyGroupArray, _PreTaskId] call OKS_fnc_Convoy_TaskTracker;
};