...
*/
	...
	while {alive _Aircraft && {Alive _X} count crew _Aircraft >= 1} do {
		...
		if(count _NearPlayers > 0) then {
			format["[AIR SCOUT] Scout Spotted: %1",_NearPlayers] spawn OKS_fnc_LogDebug;
		};
		{
			...
			if(stance _X == "PRONE" && _nearConcealment >= 1) then {
				"[AIR SCOUT] Player is prone and near concealment. Required value is 0.65" spawn OKS_fnc_LogDebug;
				_requiredValueForSpotting = 0.65;
			} else {
				"[AIR SCOUT] Player isn't prone. Required value is 0.5" spawn OKS_fnc_LogDebug;
			};
			...
			if(_ValueSpotted >= _requiredValueForSpotting) then {
				...
				format ["[AIR SCOUT] %1 was revealed (%2) by %3.",name _X,_SpottingValue,[configFile >> "CfgVehicles" >> typeOf _Aircraft] call BIS_fnc_displayName] spawn OKS_fnc_LogDebug;
			};		
		...
		if(count _NearPlayers > 0) then {
			if(!(missionNamespace getVariable ["Active_UAV_Mortar",false])) then {		
				...
				if(_nofriendlyNear) then {
					format ["[AIR SCOUT] %1 was targeted by mortar.",name _targetPlayer] spawn OKS_fnc_LogDebug;
					...
					"[AIR SCOUT] Friendly forces too close to strike. Aborting." spawn OKS_fnc_LogDebug;
				}
			};
			...
			if(!(_Careless)) then {
				...
				format ["[AIR SCOUT] %1 was spotted (%2) by %3. Not careless - Adding SAD waypoint.",name _targetPlayer,_ValueSpotted,[configFile >> "CfgVehicles" >> typeOf _Aircraft] call BIS_fnc_displayName] spawn OKS_fnc_LogDebug;
			};
		};
		sleep 5;
	};
