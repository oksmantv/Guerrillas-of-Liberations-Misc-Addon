...
if(!isNil "wecho1") exitWith {
	if((alive wecho1)) then {	
		[wecho1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupParams;
	} else {
		[wecho1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
	};
};
if(!isNil "eecho1") exitWith {
	if((alive eecho1)) then {	
		[eecho1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupParams;
	} else {
		[eecho1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
	};
};
