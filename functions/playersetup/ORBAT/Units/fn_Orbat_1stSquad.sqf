...
if(!isNil "w1a") exitWith {
	if((alive w1a)) then {	
		[w1a,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupParams;
	} else {
		[w1a,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
	};
};
if(!isNil "e1a") exitWith {
	if((alive e1a)) then {	
		[e1a,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupParams;
	} else {
		[e1a,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
	};
};
...
if(GOL_Composition isEqualTo 1) exitWith {
	if(!isNil "w1a1") exitWith {
		if((alive w1a1)) then {	
			...
			[w1a1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
		};
	};
	if(!isNil "e1a1") exitWith {
		if((alive e1a1)) then {	
			[e1a1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupParams;
		} else {
			[e1a1,_OrbatPath] spawn OKS_Fnc_Orbat_SetGroupInactive;
		};
	};
